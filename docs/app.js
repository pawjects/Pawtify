(() => {
  ...
  function renderHomePage() {
    ...
    // Split trending into categories for different sections
        ...
        <section class="overlay welcome-overlay" data-action="dismiss-overlay">
          <article class="modal welcome-modal">
            <div class="welcome-content">
              ...
              <p class="welcome-meta">&copy; ${year} Pawtify. All rights reserved.</p>
            </div>
          </article>
        </section>
      `;
  ...
  function transformArtist(raw) {
    return {
      ...
      type: raw.type || raw.description || "Artist",
      bio: raw.bio?.[0]?.text || ""
    };
  }

  async function searchSongs(query, page = 0, limit = 10) {
    const payload = await apiGet("/search", { query, page, limit });
    if (!payload.success) return [];
    return (payload.data?.songs?.results || []).map(transformSong);
  }

  async function searchArtists(query, page = 0, limit = 10) {
    const payload = await apiGet("/search", { query, page, limit });
    if (!payload.success) return [];
    return (payload.data?.artists?.results || []).map(transformArtist);
  }

  async function getSongDetails(id) {
    const payload = await apiGet("/songs", { ids: id });
  ...
  async function getSongRecommendations(id, limit = 10) {
    // New API doesn't have suggestions, search by song title instead
    const song = getSongById(id);
    if (!song) return [];
    const payload = await apiGet("/search", { query: song.title, limit });
    if (!payload.success) return [];
    const results = payload.data?.songs?.results || [];
    return results.filter(s => String(s.id) !== String(id)).slice(0, limit).map(transformSong);
  }

  async function getSongLyrics(id) {
    // New API doesn't have a lyrics endpoint; return null
    return null;
  }
  ...
  async function getTrendingSongs(limit = 10) {
    ...
    for (const artist of featuredArtists) {
      try {
        const payload = await apiGet("/search", { query: artist, page: 0, limit: 3 });
        if (payload.success) {
          const results = payload.data?.songs?.results || [];
    ...
    for (const q of moodQueries) {
      try {
        const payload = await apiGet("/search", { query: q, page: 0, limit: 4 });
        if (payload.success) {
          const results = payload.data?.songs?.results || [];
          allSongs.push(...results.map(transformSong));
        }
